...
def u2net(input_shape, out_ch, int_ch, num_classes=1):
    ...
    # print(d5.shape)       # (None, 512, 512, 64)

    """Side outputs"""
    y1 = Conv2D(num_classes, 3, padding="same" )(d5)
    # print(y1.shape)         # (None, 512, 512, 1)

    y2 = Conv2D(num_classes, 3, padding="same" )(d4)
    # print(y2.shape)          #   (None, 256, 256, 1) so upsample
    y2 = UpSampling2D(size=(2,2), interpolation="bilinear")(y2)         # (None, 512, 512, 1)

    y3 = Conv2D(num_classes, 3, padding="same" )(d3)
    y3 = UpSampling2D(size=(4,4), interpolation="bilinear")(y3)         # (None, 512, 512, 1)
    ...
    y6 = UpSampling2D(size=(32,32), interpolation="bilinear")(y6)           # (None, 512, 512, 1)

    # if we have more than one class - activation="softmax"
